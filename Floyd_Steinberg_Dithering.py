...
for c in range(img_quantised.shape[2]):    
    for row in range(h):
        for column in range(l):
            ...
            if row< h-1:
                img_quantised[row+1,column,c] += err*7/16
            if column< l-1:
                img_quantised[row,column+1,c] += err*5/16
            if (row< h-1) and (column< l-1):
                img_quantised[row+1,column+1,c] += err*1/16
            if (row>0) and (column>0):
                img_quantised[row-1,column-1,c] += err*3/16
