...
for c in range(img_quantised.shape[2]):    
    for row in range(h):
        for column in range(l):
            ...
            if row< h-1:
                img_quantised[row+1,column,c] += err*1/8
            if column< l-1:
                img_quantised[row,column+1,c] += err*1/8
            if (row< h-1) and (column< l-1):
                img_quantised[row+1,column+1,c] += err*1/8
            if (column>0):
                img_quantised[row-1,column-1,c] += err*1/8
            if row < h-2:
                img_quantised[row-2,column,c] += err*1/8
            if column < l-2:
                img_quantised[row,column-2,c] += err*1/8
